...
def draw_ladders_and_geometric_boundary_for_veering_isosig(sig, args = {}, global_drawing_scale = 12):
	if args == {}:
		...
		args = {'draw_boundary_triangulation':True, 'draw_labels': True, 'ct_lw': 0.02, 'global_drawing_scale': 12, 'style': 'geometric', 'draw_triangles_near_poles': True, 'ct_depth': -1, 'ladder_width': 10.0, 'ladder_height': 20.0, 'scale_for_CT': False} #ct_depth is the old way to try to build ct maps

	args['global_drawing_scale'] = global_drawing_scale ### hard to get this to work for all examples at once, so you can set it in the function call to get a better image
