...
def generate_flow_cycles(sig, max_length = 5, include_num_steps_up = False, monochromatic_only = False, min_length = None):
    tri, angle = isosig_to_tri_angle(sig)
    ...
    if include_num_steps_up:
        ...
        if min_length != None:
            flow_cycles = [c for c in flow_cycles if len(c[0]) >= min_length]
    else:
...
def go_up_around_edge(tet_vert_coorientations, tet, face_num, verts, fc_next_tet_index, verbose = 0):
    around_edge = []
    while True:
        ...
        next_verts.sort()
        if verbose > 1:
            print('next_tet.index()', next_tet.index(), 'next_verts', tuple(next_verts), '(b0, b1)', (b0, b1))
        ...
        if b0 in next_verts:
            ...
            assert b1 in next_verts
            face_num = b0
        tet = next_tet
        verts = next_verts
        if verbose > 1: 
            print('adding next step: (tet.index(), face_num)', (tet.index(), face_num))
        around_edge.append((tet.index(), face_num))
    return around_edge

def flow_cycle_to_dual_edge_loop(tri, angle, flow_cycle, verbose = 0): 
